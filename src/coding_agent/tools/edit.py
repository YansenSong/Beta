...
def prepare_edit_arguments(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    这个函数有意只调整 JSON shape。matching 和 file validation 仍保留在 Tool handler 中，
    以确保它们基于同一个原始 file snapshot 执行。
    """
...
# 提供一个更具描述性的 alias，方便调用方直接测试 normalization seam；
# Tool 自身仍使用标准名称 ``prepare_edit_arguments``。
...
def _find_all(haystack: bytes, needle: bytes) -> list[int]:
    ...
    while True:
        ...
        # 每次只前进一个 byte，这样 overlapping match（例如 ``aaa`` 中的 ``aa``）
        # 也会被视为重复匹配。
        start = position + 1
...
def create_edit_tool(cwd: str | Path) -> Tool[EditArgs]:
    ...
    async def edit_file(args: EditArgs, ctx: ToolExecutionContext) -> ToolResult:
        ...
        # 从末尾向前应用 replacement，可让每个 range 始终锚定在原始 snapshot 上，
        # 并从 Tool 的视角保证写入是 atomic 的。
